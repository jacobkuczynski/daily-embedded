...
Tell Claude "review today's problem". You'll get feedback covering correctness, embedded-specific code quality, what an interviewer would call out (especially around `volatile` and memory ordering), and a cleaner alternative if yours wasn't the tightest. Your attempt gets appended to `attempts/`.
